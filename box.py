import pygame
import global_functionality as functions

#implementation of 'box' every 'box' in the game is an instance of this 
...
class Box:
  def __init__(self, X_location, Y_location, game_display, box_art):
    ...
    self.isMoving = False
    #a booliean that checks if the box can move or should be remainded stationary
    self.canMove = True
...
#updates the location of the box every second and moves it
  def update(self): 
    if(self.X_location == 1300 and self.Y_location == 100):
      self.canMove = False
    if(self.isMoving):
